...
/**
 * @description: Twitter_Snowflake 主键生成算法
 * @author: cuking
 * @date: 2017/10/25
 */
...
//最小位数
    ...
    public synchronized String nextId() {
        ...
        if (null == workerId) {
            ...
            if (StringUtils.isBlank(macAddress)) {
                ...
                Object s = null;
                if (redisTemplate.hasKey(GxConfig.SNOW_FLAKE_WORKER_MAP)) {
                    s = redisTemplate.opsForHash().get(GxConfig.SNOW_FLAKE_WORKER_MAP, macAddress);
                }
                if (null == s) {
                    long increment = redisTemplate.opsForValue().increment(GxConfig.SNOW_FLAKE_WORKER, 1L);
                    workerId = increment;
                    //缓存到数据库
                    redisTemplate.opsForHash().put(GxConfig.SNOW_FLAKE_WORKER_MAP, macAddress, increment + "");
                } else {
                    //读取缓存的 worker Id
    ...
    public String generateFormatId(String time) {
        try {
            long increment = redisTemplate.opsForValue().increment(GxConfig.ZM_ID, 1L);
            int i = (int) increment;
            return coverZero(MINFULL, ++i, time);
        } catch (Exception e) {
            logger.error("生成ID 出错!!!", e);
        }
